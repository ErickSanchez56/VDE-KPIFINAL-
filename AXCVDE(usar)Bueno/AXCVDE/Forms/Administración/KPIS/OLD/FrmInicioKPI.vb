...
#Region "RECEPCION"
    ...
    Private Sub llenarGraficaDockToStock()
        Try
            ...
            If tablaPromedio.Rows.Count > 0 Then
                'Gauges dashboard
                'digitalGaugeDiasColocacion.Text = tablaPromedio.Rows(0).Item("Dias")
                ...
                DigitalGauge4.Text = tablaPromedio.Rows(0).Item("Horas")
    ...
    Private Sub ResultadoListaUsuarioxRecepcion(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            GridControl3.DataSource = promediousuarioxrecepcion
        Catch ex As Exception
    ...
    Private Sub ResultadoLista2(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            If Not pResultado.Estado Then
                ...
                dgvDetalleRecepcionProm.DataSource = Nothing
                Return
            End If
            ...
            dgvDetalleRecepcionProm.DataSource = dt
            dgvView2.BestFitColumns()
            ...
            If tablaPromedio.Rows.Count > 0 Then
                For Each row As DataRow In tablaPromedio.Rows
                    If row.Item("Recibos").ToString = row.Item("Recibos").ToString Then
                        LblRecibosdeCompra.Text = row.Item("Recibos").ToString
    ...
    Private Sub llenarGraficaDockToStock(ByRef ds As DataSet)
        Try
            If ds.Tables(0).Rows.Count > 0 Then
                'Gauges dashboard´´PARTIDA
                DigitalGaugeRecPartida.Text = ds.Tables(0).Rows(0).Item("Dias")
                DigitalGaugeHorasRec.Text = ds.Tables(0).Rows(0).Item("Horas")
                DigitalGaugeMinutosRecPartida.Text = ds.Tables(0).Rows(0).Item("Minutos")

                ''--TOTAL
                DigitalGauge8.Text = ds.Tables(0).Rows(0).Item("Dias")
                DigitalGauge7.Text = ds.Tables(0).Rows(0).Item("Horas")
                DigitalGauge6.Text = ds.Tables(0).Rows(0).Item("Minutos")
            End If
        Catch ex As Exception
            XtraMessageBox.Show(ex.Message, "AXC")
        End Try
    ...
    Private Sub llenarGraficas(ByVal dt As DataTable)
        Try
            If dt.Rows.Count > 0 Then
                ChartControlPromedioPorPartida.Titles.Clear()
                ChartControlPromedioPorPartida.Series.Clear()
                ChartControlPromedioPorPartida.DataSource = dt
                ...
                For Each rowAlm As DataRow In dt.Rows
                    ...
                    For Each rowOcupados As DataRow In dt.Rows
                        ...
                        End If
                    Next
                    ChartControlPromedioPorPartida.Series.Add(series(contador))
                    contador += 1
                Next
                'Cast the chart's diagram to the XYDiagram type, to access its axes.
                Dim diagram As XYDiagram = CType(ChartControlPromedioPorPartida.Diagram, XYDiagram)
...
#Region "Almacen"
            ...
            If tablaPromedio.Rows.Count > 0 Then
                For Each row As DataRow In tablaPromedio.Rows
                    If row.Item("Almacén") = "CEDIS" Then
                        ArcScaleComponent1.Value = row.Item("Ocupación")

                        LblOcupacionActualdelAlmacen.Text = row.Item("Ocupación").ToString
                        LabelControl32.Text = row.Item("Ocupación").ToString
    ...
    Private Sub llenarGraficasAlmacenamientoPestañaAlmacenamiento(ByVal tablaPromedio As DataTable, ByVal tablaGeneral As DataTable, ByVal tablaDistintosAlmacenes As DataTable)
        Try
            If tablaPromedio.Rows.Count > 0 Then
                ...
                For Each row As DataRow In tablaPromedio.Rows
                    If row.Item("Almacén") = "CEDIS" Then
                        'ArcScaleComponent5.Value = row.Item("Ocupación")
                        LabelControl29.Text = row.Item("Ocupación").ToString
    ...
    Private Sub llenarGraficaOrdenesPrecisionInv()
        Try
            ...
            Dim tablaGeneral = resultado.Tablas.Tables(2)
            If tablaPromedio.Rows.Count > 0 Then
                ArcScaleComponent6.Value = tablaPromedio.Rows(0).Item("Precisión")
                LabelControl30.Text = tablaPromedio.Rows(0).Item("Precisión")
            End If
...
#Region "Produccion"
    ...
    Private Sub ResultadoListaProductosPorOrdenProd(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            ChartControl4.DataSource = TablaCantidadRegOrden
            ChartControl4.Series.Clear()
            ChartControl4.Titles.Clear()
            ChartControl4.Titles.Add(New ChartTitle() With {.Text = "Top 10 Órdenes con más cantidades registradas"})
            ChartControl4.Legend.Title.Text = ""
            ChartControl4.SeriesDataMember = "Ordenproducción"
            ChartControl4.SeriesTemplate.ValueDataMembers.AddRange(New String() {"CantidadRegistradaPorOrden"})
            ChartControl4.SeriesTemplate.ArgumentDataMember = "Ordenproducción"
            ...
            DigitalGauge16.Text = TablaPromedioYTotal.Rows(0).Item("CantidadDeProductoTotal")
    ...
    Private Sub ResultadoListaTiempoRegistroPT(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            Dim TablaDetalleTiemposurtidoxpartida As DataTable
            TablaDetalleTiemposurtidoxpartida = pResultado.Tablas.Tables(3)
            If dt.Rows.Count < 1 Then
                XtraMessageBox.Show("Sin información en el sistema", "AXC", MessageBoxButtons.OK)
            ...
            'Gauges
            'DigitalGauge29.Text = TablaTiempoRegistroPromedio.Rows(0).Item("Días")
            DigitalGauge28.Text = TablaTiempoRegistroPromedio.Rows(0).Item("Horas")
            DigitalGauge27.Text = TablaTiempoRegistroPromedio.Rows(0).Item("Minutos")
            ' DigitalGauge32.Text = TablaDetalleTiemposurtidoxpartida.Rows(0).Item("Días")
            DigitalGauge31.Text = TablaDetalleTiemposurtidoxpartida.Rows(0).Item("Horas")
            DigitalGauge30.Text = TablaDetalleTiemposurtidoxpartida.Rows(0).Item("Minutos")
    ...
    Private Sub ResultadoLista3(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            LblOrdProduccion.Text = row.Item("Ordenes Completas")
            LabelComponent1.Text = row.Item("Ordenes Completas")
            'ArcScaleComponent3 = row.Item("Ordenes Completas")
...
#Region "Embarques"
    ...
    Private Sub ResultadoListaOrdSurtidasEmbarcadas(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            chartTitle1.Text = "Porcentaje Órdenes Embarcadas"
    ...
    Private Sub ResultadoLista6(prmBusqueda As String, prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            'ChartControl11.Titles.Add(New ChartTitle() With {.Text = "Tiempo Promedio Surtido"})
    ...
    Private Sub ResultadoLista(prmBusqueda2 As Date, prmBusqueda3 As Date)
        Try
            ...
            LblOrdEmbarques.Text = dt.Rows(0).Item("Totalembarques")

            LabelComponent2.Text = dt.Rows(0).Item("TotalembarquesaTiempo")
            'ArcScaleComponent4 = dt.Rows(0).Item("TotalembarquesaTiempo")
