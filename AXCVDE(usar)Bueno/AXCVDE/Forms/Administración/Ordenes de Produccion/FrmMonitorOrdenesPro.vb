...
Public Class FrmMonitorOrdenesPro
...
#Region "EVENTOS"
    ...
    Public Sub OnIrLiberarRowClick(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim menuItem As DXMenuItem = TryCast(sender, DXMenuItem)
            Dim ri As RowInfo = TryCast(menuItem.Tag, RowInfo)
            Dim FrmLiberar As New FrmLiberacionOrdenProduccion
            FrmLiberar.MdiParent = FrmMenuPrincipal.XtraTabbedMdiManager1.MdiParent
            FrmLiberar.pOrdenProd = ri.View.GetRowCellDisplayText(ri.RowHandle, "OrdenProd")
    ...
    Public Sub OnIrAutorizaRowClick(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            ri.View.SetRowCellValue(ri.RowHandle, "Autorizada", "AUTORIZADA")
...
#Region "METODOS"
    ...
    Public Sub cargaOrdenesProd(ByVal prmBusqueda As String, ByVal prmDias As String)
        Try
            ...
            Cursor.Current = Cursors.WaitCursor
            pResultado = Cls.ListaOrdenesProduccionMontitor(prmDias, My.Settings.Estacion, DatosTemporales.Usuario)
            Cursor.Current = Cursors.Default
            ...
            Cls = New clsOrdenProd

            Cursor.Current = Cursors.WaitCursor
             pResultado = Cls.ListaOrdenesProduccionMontitorDetalle(prmDias, My.Settings.Estacion, DatosTemporales.Usuario)
            Cursor.Current = Cursors.Default
            ...
            Dim keyColumn As DataColumn = ds.Tables("Encabezado").Columns("OrdenProd")
            Dim foreignKeyColumn As DataColumn = ds.Tables("Detalle").Columns("OrdenProd")
            ds.Relations.Add("DetalleOrdenesProduccion", keyColumn, foreignKeyColumn)
        ...
        ElseIf e.Button.Properties.Tag = "Excel" Then
            ...
            If sv.ShowDialog() = DialogResult.OK And sv.FileName <> Nothing Then
                If sv.FileName.EndsWith(".xlsx") Then
                    ...
                    advOptions.SheetName = "Partidas de orden producción"
